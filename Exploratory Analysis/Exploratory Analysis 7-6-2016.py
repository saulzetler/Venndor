...
df_original = pd.read_excel('C:\\Users\\burksa\\Documents\\Python Scripts\\Venndor\\Exploratory Analysis\\Analytics Sheets.xlsx', 'Market tests')
...
df_test = df_data[['OfferMade', 'PricePoints']].groupby('PricePoints').agg(np.mean)
#%%
...
regressionSummary = pd.DataFrame(linReg, index=[['Slope', 'Int', 'RValue', 'Pvalue', 'Stderr']])
...
ax = ax.map(sns.kdeplot, 'Offer'  # ,bins=np.arange(len( set( df_data['PriceRatio'] ) )+1)-0.5
            , shade=True, linewidth=5).add_legend()
...
ax = sns.FacetGrid(data=df_data, hue='PricePoints')
ax = ax.map(sns.kdeplot, 'PriceRatio'  # ,bins=np.arange(len( set( df_data['PriceRatio'] ) )+1)-0.5
            , shade=True, linewidth=5).add_legend()

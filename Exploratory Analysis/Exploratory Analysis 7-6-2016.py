...
df_grouped = df_data[['SellingPrice','Difference']].astype('float').groupby('SellingPrice',as_index=False).aggregate(np.mean)
#%%
...
ax = sns.stripplot(data=df_data,x='SellingPrice',y='Difference',hue='OfferMade',split=True)

ax = sns.lmplot(data=df_grouped,x='SellingPrice',y='Difference')

ax = sns.FacetGrid(data=df_data,col='Product',col_wrap=5)
ax = ax.map(plt.hist,'Offer')
